# Qiven Foundation Architecture (bootstrap + amendments snapshot, museum)

> Museum (2026-09-26 PR4 doc-repair): the bootstrap architecture with its
> later accretions, preserved verbatim from the live path
> `docs/architecture/foundation.md`. Its admission wording ("broadly
> useful", §15's convenience bar) was superseded by ADR-0024 (one real
> consumer suffices when the semantic owner is clear); the current active
> contract, which retains the valid dependency/ownership/representation/
> platform/testing provisions, lives at the live path. Historical
> quotations only — not current law.
